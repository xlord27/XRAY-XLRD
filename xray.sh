...
wget -O /usr/local/bin/xray "https://raw.githubusercontent.com//xlord27/XRAY-XLRD/main/xray.linux.64bit"
chmod +x /usr/local/bin/xray

# generate certificates
mkdir /root/.acme.sh
curl https://raw.githubusercontent.com/xlord27/XRAY-XLRD/main/acme.sh -o /root/.acme.sh/acme.sh
...
sed -i '$ ilocation = /funy' /etc/nginx/conf.d/xray.conf
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
...
wget -O user-vmess "https://${Server_URL}/user-vmess.sh" && chmod +x user-vmess
...
wget -O trial-tr "https://${Server_URL}/menu-system.sh" && chmod +x menu-system
# // OTHER FILES
